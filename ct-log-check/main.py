...
- **The issuers** — grouped by CA, never by the intermediate that
  signed (`CN=R3` and `CN=E5` are one Let's Encrypt): your main CA
  and *everyone else who ever issued for the domain* — the headline
  of this report.  A CA you don't recognize in that list is either
  history (a rotation) or a question (who ordered that cert?).
  A precertificate and its final certificate are folded into the
  one issuance they are, so no count is doubled.
...
- **CAA vs the actual issuers** — the policy half nobody reads:
  ...
  the curated mapping doesn't recognize ⚪ (check by hand).  A
  domain with no CAA record at all has no list to be outside of —
  its CAs are reported as unconstrained, never as violations.  The
  nuance is said aloud: CAA governs from the moment it was
  published — a live certificate may legitimately predate your own
  policy.
- **An old feed, read honestly** — a feed whose newest issuance is
  months old is either crt.sh's cap for very large domains or a
  dormant domain; only the feed size tells them apart, and the
  report says which rather than guessing.
...
# crt.sh caps its JSON at ~5000 rows for very large domains. A feed
# whose newest issuance is months old is either that cap — or a
# dormant domain; only the size tells them apart, and this is the
# line below which the cap is never claimed.
CAP_SUSPECT_ROWS = 2500

# One DNS label, then the domain shape: at least two labels, no empty
# ones ("example..com"), no leading/trailing hyphen.
...
def make_session() -> requests.Session:
    ...
    # 404 is on the list on purpose: under load crt.sh answers a
    # transient 404 (an nginx 502 page wearing a 404 status) and the
    # next identical request succeeds — verified live.
...
def parse_rows(rows: list[dict], domain: str) -> list[dict]:
    """crt.sh rows → one record per unique (certificate, name) with
    issuer, validity and wildcard flag — names filtered to the domain.

    A precertificate and its final certificate are two crt.sh rows —
    different ids, same serial from the same CA — but **one**
    issuance.  Deduping on (issuer_ca_id, serial_number) folds the
    pair; without it every count roughly doubles (isc.org live: 3914
    rows, 2004 certificates).
    """
    out = []
    ...
    for row in rows:
        if not isinstance(row, dict):
            continue            # a payload shape crt.sh never sends
        cert_id = str(row.get("id", ""))
        serial = str(row.get("serial_number") or "").strip().lower()
        ca_id = str(row.get("issuer_ca_id") or "")
        # the identity of the certificate, not of the log entry
        cert_key = f"{ca_id}:{serial}" if serial else f"id:{cert_id}"
        ...
        for name in names:
            ...
            if (cert_key, name) in seen:
                continue
            seen.add((cert_key, name))
            out.append({
                "cert_id": cert_id, "cert_key": cert_key,
...
def by_certificate(records: list[dict]) -> list[dict]:
    """One record per certificate. `records` are (certificate, name)
    pairs, so counting them counts a multi-name cert several times —
    issuer, month and live counts all want this list instead."""
    seen: set[str] = set()
    out = []
    for r in records:
        key = r.get("cert_key") or r["cert_id"]
        if key in seen:
            continue
        seen.add(key)
        out.append(r)
...
ISSUER_CAA_MAP = [
    ...
    # "O=SSL Corporation, CN=Cloudflare TLS Issuing ECC CA 3" is an
    # SSL.com-operated intermediate wearing Cloudflare's name — the
    # CAA identifier is ssl.com, and this entry must stay above the
    # cloudflare one or the CN wins the match.
    (r"\bssl\.com\b|ssl corporation", "ssl.com"),
...
def caa_climb_chain(domain: str) -> list[str]:
    ...
    while len(labels) >= 2:
        ...
        # the registrable domain is the last rung: two labels, or
        # three whose tail is a known second-level suffix. Climbing
        # past it would ask a public suffix (`co.uk`) for a policy.
        if len(labels) == 2:
            break
        if len(labels) == 3 and \
...
def issuer_org(issuer: str) -> str:
    """The O= of a crt.sh issuer DN — the CA itself, not the
    intermediate that signed: "C=US, O=Let's Encrypt, CN=R11" is
    Let's Encrypt, and so are R3, E5 and X3."""
    # O="Cloudflare, Inc." — a quoted value carries its own commas
    m = re.search(r'(?:^|,)\s*O=(?:"([^"]*)"|([^,]+))', issuer or "")
    if not m:
        return (issuer or "").strip()
    return (m.group(1) if m.group(1) is not None
            else m.group(2)).strip()


def ca_group(issuer: str) -> tuple[str, str]:
    """(key, label) for grouping issuances by CA — the curated CAA
    domain when the mapping knows the CA (canonical across renamed
    intermediates), else the O=. Never the CN: that is the
    intermediate, and a census keyed on it lists one CA a dozen
    times (isc.org live: 48 intermediates, 10 CAs)."""
    org = issuer_org(issuer)
    caa = map_issuer_to_caa(issuer)
    return (caa or org.lower() or "unknown"), (org or "unknown")


def group_by_ca(certs: list[dict]) -> list[dict]:
    """Certificates per CA, most first — one row per CA with the
    number of intermediates it signed through."""
    groups: dict[str, dict] = {}
    for r in certs:
        key, label = ca_group(r["issuer"])
        g = groups.setdefault(key, {
            "caa": map_issuer_to_caa(r["issuer"]),
            "labels": Counter(), "certs": 0, "issuers": set()})
        g["labels"][label] += 1
        g["certs"] += 1
        g["issuers"].add(r["issuer"])
    out = [{"ca": g["labels"].most_common(1)[0][0], "caa": g["caa"],
            "certs": g["certs"], "intermediates": len(g["issuers"])}
           for g in groups.values()]
    out.sort(key=lambda g: (-g["certs"], g["ca"]))
    return out
...
def caa_verdicts(records: list[dict], caa: dict) -> list[dict]:
    ...
    # issuewild joins issue for the allow set; an `issue ";"` alone
    # leaves it empty — the forbid-all shape, where every mapped CA
    # lands outside without a special case.
    allowed = {d for d in policy["issue"] + policy["issuewild"]
               if d}
    now = dt.datetime.now(dt.timezone.utc)
    live = [r for r in by_certificate(records)
            if r["not_before"] <= now <= r["not_after"]]
    if not live:
        return [{"kind": "no live certs"}]
    verdicts = []
    for g in group_by_ca(live):
        if caa["status"] != "ok":
            # no CAA record anywhere up the tree: there is no list to
            # be outside of, and saying "outside" would cry wolf at a
            # domain whose only fault is publishing no policy.
            kind = "unconstrained"
        elif g["caa"] is None:
            kind = "unrecognized"
        elif g["caa"] in allowed:
            kind = "allowed"
        else:
            kind = "outside"
        verdicts.append({"kind": kind, "ca": g["ca"], "caa": g["caa"],
                         "certs": g["certs"]})
    return verdicts
...
def analyze(records: list[dict], feed_rows: int | None = None) -> dict:
    now = dt.datetime.now(dt.timezone.utc)
    certs = by_certificate(records)
    by_issuer: Counter = Counter(r["issuer"] for r in certs)
    by_ca = group_by_ca(certs)
    months: Counter = Counter(
        r["not_before"].strftime("%Y-%m") for r in certs)
    wildcards = [r for r in records if r["wildcard"]]
    valid_now = [r for r in records
                 if r["not_before"] <= now <= r["not_after"]]
    valid_certs = [r for r in certs
                   if r["not_before"] <= now <= r["not_after"]]
    ...
    others = [(i, c) for i, c in by_issuer.most_common()
              if i != main_issuer]
    # a burst: more issuances in the last 30 days than three times
    # the monthly average of the last year
    recent = [r for r in certs
              if (now - r["not_before"]).days <= 30]
    year = [r for r in certs
            if (now - r["not_before"]).days <= 365]
    avg = len(year) / 12 if year else 0
    burst = len(recent) > 3 * max(1.0, avg)
    # A feed whose newest issuance is months old is either the crt.sh
    # cap (very large domains, oldest ~5000 rows kept) or a dormant
    # domain — and only the feed size tells them apart. Claiming the
    # cap for a twelve-row feed would be a confident lie, so the cap
    # is named only above CAP_SUSPECT_ROWS; below it the report says
    # "dormant" instead.
    newest = max((r["not_before"] for r in certs), default=now)
    stale_days = (now - newest).days
    feed_stale = stale_days > 90
    feed_size = feed_rows if feed_rows is not None else len(certs)
    feed_capped = feed_stale and feed_size >= CAP_SUSPECT_ROWS
    return {
        "records": len(records),
...
def build_table_event(a: dict) -> dict:
    rows = [[c["ca"], c["certs"], c["intermediates"]]
            for c in a["by_ca"][:15]]
    return {"type": "table",
            "columns": ["CA", "certificates", "intermediates"],
            "rows": rows}
...
def build_markdown(domain: str, a: dict, caa: dict | None = None,
    ...
    head = (f"Domain: **{domain}** · {a['certificates']} "
            f"certificate(s) over {a['names']} name(s)")
    # under a capped feed the valid-now count is an artefact of the
    # cap, not a fact about the domain — never print it as one
    head += ((" · the valid-now count is **unavailable**: the feed "
              "is capped (see Timeline)") if a["feed_capped"]
             else f" · {a['valid_certs']} valid right now")
    out = ["# CT Log Check — Report\n", head + "\n"]
    ...
    out.append("## Issuers\n")
    main_ca = a["by_ca"][0] if a["by_ca"] else None
    if main_ca:
        out.append(f"- main: **{main_ca['ca']}**"
                   + (f" (`{main_ca['caa']}`)" if main_ca["caa"]
                      else "")
                   + f" — {main_ca['certs']} certificate(s) via "
                     f"{main_ca['intermediates']} intermediate(s)")
    if a["other_cas"]:
        out.append(f"- everyone else who ever issued for "
                   f"`{domain}`:")
        for c in a["other_cas"][:10]:
            out.append(f"  - {c['ca']}"
                       + (f" (`{c['caa']}`)" if c["caa"] else "")
                       + f": {c['certs']} certificate(s) via "
                         f"{c['intermediates']} intermediate(s)")
        out.append("")
    ...
    out.append("")
    out.append(f"Grouped by **CA**, not by the intermediate that "
               f"signed: `CN=R3`, `CN=E5` and `CN=X3` are one Let's "
               f"Encrypt, and a census keyed on the CN would list "
               f"one CA many times ({len(a['by_issuer'])} "
               f"intermediate(s) here, {len(a['by_ca'])} CA(s)). "
               f"findings.json keeps the per-intermediate counts.")
    out.append("")
    ...
    if months:
        ...
        out.append(f"- first issuance in the feed: **{first}**, "
                   f"latest: **{last}**")
        out.append(f"- last 30 days: {a['recent_30d']} "
                   "certificate(s)")
        if a["burst"]:
        ...
        if a["feed_capped"]:
            out.append(f"- ⚫ the newest issuance in this feed is "
                       f"{a['feed_stale_days']} days old across "
                       f"{a['feed_rows']} feed entries — crt.sh "
                       "**caps its JSON output for very large "
                       "domains**, and the cap keeps the oldest "
                       "rows; the recent history (and every 'valid "
                       "now' count) is missing. For a domain this "
                       "size, watch issuances via the crt.sh web UI "
                       "or a dedicated CT monitor.")
        elif a["feed_stale"]:
            out.append(f"- ⚫ no certificate has been issued for "
                       f"{a['feed_stale_days']} days. The feed is "
                       f"small ({a['feed_rows']} entries, far under "
                       "crt.sh's cap for large domains), so this is "
                       "a **dormant domain, not a truncated feed** — "
    ...
    out.append("## Names\n")
    if a["feed_capped"]:
        out.append(f"- {a['names']} distinct name(s) in the rows the "
                   "feed returned; how many are still covered by a "
                   "valid certificate **cannot be told from a capped "
                   "feed**.")
    else:
        out.append(f"- {a['names']} distinct name(s) in the certs; "
                   f"{len(a['valid_names'])} still covered by a "
                   "valid certificate.")
    ...
    if verdicts is not None:
        ...
        for v in verdicts:
            ...
            if v["kind"] == "no live certs":
                ...
                break
            who = f"**{v['ca']}** ({v['certs']} live cert(s))"
            if v["kind"] == "allowed":
                out.append(f"- ✅ {who} → `{v['caa']}` — allowed")
            elif v["kind"] == "outside":
                out.append(f"- 🔴 {who} → `{v['caa']}` — **outside "
                           "the CAA list**: issued before the policy "
                           "was set, or a gap in it")
            elif v["kind"] == "unconstrained":
                out.append(f"- · {who}"
                           + (f" → `{v['caa']}`" if v["caa"] else "")
                           + " — there is no policy to be outside "
                             "of; nothing constrains this issuance")
            else:
                out.append(f"- ⚪ {who} — an issuer the curated "
                           "mapping doesn't recognize; check by hand "
                           f"(`dig CAA {domain}` + the cert's "
                           "issuer)")
        if caa["status"] == "ok" and verdicts and \
                verdicts[0]["kind"] not in ("failed",
                                            "no live certs"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:                        # IDN → punycode: the form crt.sh
        domain = domain.encode("idna").decode("ascii")   # and DNS
    except (UnicodeError, UnicodeDecodeError):           # both speak
        pass                    # not encodable — DOMAIN_RE rejects it
    ...
    if not records:
        ...
        return 1
    a = analyze(records, feed_rows=len(rows))
    ...
    valid = ("valid-now unavailable (capped feed)"
             if a["feed_capped"] else f"{a['valid_certs']} valid now")
    summary = (f"{a['certificates']} certificate(s) · "
               f"{a['names']} name(s) · "
               f"{len(a['by_ca'])} CA(s) · {valid}")
    status(summary)
    log(f"← {summary}")
    return 0
