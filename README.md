...
| [`port-check/`](port-check) | TCP port scan of a host you own — the common-service set (~110 ports), your own spec, or the full range — with banner grabbing, best-effort service guessing and an exposure-group report. Gated by an ownership confirmation; TCP only, nothing beyond the banner. | None — standard library only |
