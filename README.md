...
| [`exif-inspect/`](exif-inspect) | See what your photos say about you — the EXIF privacy inventory with GPS coordinates spelled out and map-linked, camera, software, dates — plus optional EXIF-free clean copies (orientation baked in first, originals untouched). | Pillow |
...
| [`a11y-check/`](a11y-check) | The accessibility pass a page deserves — WCAG text contrast computed from static CSS (the color-palette engine, large text at 3:1), forms without labels, heading skips, lang on html, ARIA roles missing their state, aria-hidden on focusable elements, positive tabindex, empty and "click here" links, tables without th, landmarks and the skip link. One fetch like Page SEO Audit; what static analysis cannot see is reported as not checked, never as passed. | requests, lxml |
| [`dmarc-report/`](dmarc-report) | What the whole internet sees in your mail — aggregate RUA reports (.xml, .xml.gz, .zip bundles with several reports inside) from every receiver merged into one picture: who sends as you, SPF/DKIM alignment per source as the receiver evaluated it (policy_evaluated, never re-guessed), forwarders behind the failures that aren't spoofing, spoof candidates, and whether the domain is ready for p=none → quarantine → reject. Offline by default; PTR resolution is the only opt-in network. The feedback-loop act of the mail story. | None — standard library only |
...
| [`ct-log-check/`](ct-log-check) | Every certificate ever issued for a domain — the crt.sh transparency feed as a timeline (issuances per month, chart), the issuer census with "everyone else who ever issued" as the headline, wildcards, names still covered by a valid certificate, the burst flag — and the honest note when crt.sh's JSON cap for very large domains hits (the feed's newest issuance months old) instead of a misleading "valid now: 0". Plus the CAA policy (via DNS-over-HTTPS, parents climbed like a CA does) compared with the CAs holding live certificates: allowed / outside the list / unrecognized, with the predates-your-policy nuance said aloud. The historical twin of TLS Audit's live certificate. | requests |
...
| [`dep-audit/`](dep-audit) | Known vulnerabilities in the project's own dependencies — the lockfiles' pinned versions (requirements, poetry, npm/yarn/pnpm, composer, go.mod, Gemfile, Cargo — 9 formats) checked against OSV's querybatch (free, keyless, batched of 100), advisory details with CVE aliases and the nearest fixed version fetched in parallel. Unpinned ranges are listed honestly as not checked, never guessed at. The third look at the same tree after secret-scan and wp-audit. | requests, pyyaml |
